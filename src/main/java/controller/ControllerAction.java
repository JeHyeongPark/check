...
public class ControllerAction 
  ...
  //경로에 맞는 CommandPro.properties파일을 불러옴
    ...
    System.out.println("불러온경로="+props); //경로명 및 파일명 확인
  ...
  //명령어와 처리클래스의 매핑정보를 저장할 Properties객체 생성
    ...
    FileInputStream f = null;//파일불러올때 (영문데이터)
     ...
     //객체를 하나씩 꺼내서 그 객체명으로 Properties 객체에 저장된 객체를 접근
     Iterator keyiter = pr.keySet().iterator();
     
     while(keyiter.hasNext()){
       //요청한 명령어를 구하기위해
       String command=(String)keyiter.next();
       System.out.println("command="+command); // /list.do 실행 확인
    ...
    //(23/01/11) 사용자의 요청을 분석해서 해당 작업을 처리
    private void requestPro(HttpServletRequest request,
    		                HttpServletResponse response) 
    throws ServletException, IOException {
    
     String view=null; //요청 명령어에 따라서 이동할 페이지 저장
     // /list.do=action.ListAction, 
     // /writeForm.do=action.WriteFotmAction, ...
     //ListAction com=null;
     //WriteFormAction com=new WriteFormAction; 
     CommandAction com=null; //어떠한 자식클래스의 객체라도 부모형으로 형변환 가능
     ...
     try {
      ...
      // /JSpBoard2/list.do
      // /JspBoard2 
      //  => 문자열을 비교 했을 때 일치하는 위치 묻는 것 (indexOf) : 0인경우 처음부터 같은거
      if(command.indexOf(request.getContextPath())==0) {//~ContextPath와 같아지는부분
    	 command=command.substring(request.getContextPath().length());
    	 System.out.println("실질적인 command="+command);
    	 // /list.do
      }
      //요청명령어 : /list.do -> action.ListAction객체 => requestPro()를 호출하기 위해
      com=(CommandAction)commandMap.get(command); //(부모형) 자식형
      System.out.println("com="+com); //=action.LostAction@주소
     ...
     RequestDispatcher dispatcher=
    		 request.getRequestDispatcher(view); // "/ist.jsp"

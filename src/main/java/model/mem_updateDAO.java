...
public class mem_updateDAO {
	...
	public mem_updateDTO getArticle(String mem_id) {
		...
		try {
			...
			sql ="select * from mem_log where mem_id=?";
	...
	//상태 변경
		public int updateMemState (mem_updateDTO order) {
			...
			try {
				con=pool.getConnection();
				sql="update mem_log set mem_pwd=?,mem_addr1=?,mem_addr2=?,mem_phone=?,mem_email=?,mem_zipcode=? where mem_id = ?";
				pstmt=con.prepareStatement(sql);
		...
		public int deleteMemState (String mem_id) {
			...
			try {
				con=pool.getConnection();
				sql="delete from mem_log where mem_id=? ";
				pstmt=con.prepareStatement(sql);
				pstmt.setString(1, mem_id);
		...
		//비밀번호 초기화
			public int resetMemState (mem_updateDTO order) {
				...
				try {
					con=pool.getConnection();
					sql="update mem_log set mem_pwd=? where mem_id = ?";
					pstmt=con.prepareStatement(sql);
					pstmt.setString(1,order.getMem_pwd());

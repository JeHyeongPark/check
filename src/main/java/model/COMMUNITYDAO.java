...
//2.글목록보기에 대한 메서드구현->레코드 한개이상->한 페이지당 10개씩 끊어서 보여준다.
	...
	public List getArticles(int start, int end) {// getMemberList(int start,int end){
		...
		try {
			 ...
			 */
			// sql="select * from com_post order by post_num desc";
...
*/
			...
			sql += " post_view)values(?,?,?,?,?,0)";
			//sql += " post_view,count,admin_id)values(?,?,?,?,?,0,?,?)"; before
			...
			//pstmt.setInt(6, post_view);// 조회수 default->0
			//pstmt.setInt(7, article.getCount()); // 댓글수
			//pstmt.setString(8, article.getAdmin_id());

			// 댓글
			// pstmt.setInt(5,article.getReply_num());
			// pstmt.setString(5,article.getReply_cnt));
			// pstmt.setTimestamp(5,article.getReply_date());
	...
	//댓글 상세 보기
		public List<COMMUNITYreplyDTO> replyDetail (int post_num) {
			List<COMMUNITYreplyDTO> replyList=null; //찾을 레코드를 담을 객체 선언
			...
			try {
				...
				rs=pstmt.executeQuery();
				if(rs.next()==true){
					replyList=new ArrayList<COMMUNITYreplyDTO>(); //왜 제네릭이 필요?
					do {
	...
	// 4.게시판의 글쓰기 및 답글쓰기
	/*
	 * public void insertArticle (boardDTO article) { // 신규글 or 답변글 int
	 * post_num=article.getPost_num(); int number=0;// 데이터 저장을 위한 게시물 번호
	 * System.out.println("insertArticle 메서드의 내부 호출");
	 * 
	 * try { con=pool.getConnection(); sql="select max(post_num) from qna_post";
	 * pstmt=con.prepareStatement(sql); rs=pstmt.executeQuery(); //1) 신규글 일 때 if
	 * (rs.next()) { // 기존 레코드 데이터가 있으면 number=rs.getInt(1)+1; //number(0)+1 }else {
	 * //기존 데이터가 없는 경우 1부터 시작한다는 뜻 number=1; } article.setPost_num(number);
	 * 
	 * sql="insert into com_post(post_num,mem_id,post_title,post_cnt,post_date,";
	 * sql+=" post_view,count,admin_id)values(?,?,?,?,?,?,?,?)";
	 * pstmt=con.prepareStatement(sql); pstmt.setInt(1, article.getPost_num());
	 * pstmt.setString(2, article.getMem_id()); pstmt.setString(3,
	 * article.getPost_title()); pstmt.setString(4, article.getPost_cnt());
	 * pstmt.setTimestamp(5, article.getPost_date()); pstmt.setInt(6,
	 * article.getPost_view()); pstmt.setInt(7, article.getCount());
	 * pstmt.setString(8, article.getAdmin_id()); int insert=pstmt.executeUpdate();
	 * System.out.println("게시판 글쓰기, 답댓글 성공 확인: "+insert); }catch(Exception e) {
	 * System.out.println("insertArticle() 에러 발생: "+e); }finally {
	 * pool.freeConnection(con, pstmt, rs); } }
	 */
		...
		try {
			...
			if (rs.next()) {// 보여주는 결과가 있다면
				...
				do {
					...
					//article.setCount(rs.getInt("count")); // 댓글수

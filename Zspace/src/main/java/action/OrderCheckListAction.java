...
public class OrderCheckListAction implements CommandAction {
	...
	public String requestPro(HttpServletRequest request, HttpServletResponse response) throws Throwable {	 
	 ...
	 if(startDate !=null && endDate !=null) {
		 ...
		 System.out.println("OrderCheckListAction의 orderList=>"+orderList);
	 } 
	  /*
	   * List<OrderCheckDTO> orderList = new ArrayList<OrderCheckDTO>();	 
	   if(startDate !=null && endDate !=null) {
		 orderList = ocdao.getArticelSearchDate(startDate,endDate);
		 System.out.println("OrderCheckListAction의 orderList=>"+orderList);
	 }  */
...
//2.처리한 결과를 공유(서버메모리에 저장)->이동할 페이지에 공유해서 사용(request)
	 ...
	 /**
	 exchangeDAO ocdao=new exchangeDAO();
	 if (startDate != null && endDate != null) {ß
		    orderList = ocdao.orderCheck(mem_id);
		    System.out.println("OrdercheckDAO의 orderList=>"+orderList);
		}*/ 